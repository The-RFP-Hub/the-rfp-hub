...
export function start(options: SpawnOptions): ManagedChild {
  ...
  // EVERY error sink is attached before anything can throw. A spawn failure (ENOENT, EMFILE)
  // arrives as an async `error` event one tick after `spawn` returns with no pid — registering
  // the listener only on the success path turns that report into an uncaught exception that kills
  // the runner mid-run. The same holds for the log stream (a full disk emits `error` on the
  // WriteStream) and the stdio pipes: none of them may be left an unhandled `error` emitter.
  const exited = new Promise<number | null>((resolve) => {
    ...
    child.once("error", (err) => {
      tail = `${tail}\n[processes] ${options.name}: ${err.message}\n`.slice(-TAIL_LIMIT);
      log.end();
      resolve(null);
    });
  });
  log.on("error", (err) => {
    tail = `${tail}\n[processes] log stream for ${options.name} failed: ${err.message}\n`.slice(
      -TAIL_LIMIT,
    );
  });
...
export async function stop(managed: ManagedChild): Promise<void> {
  // A dead LEADER says nothing about the GROUP: pnpm and tsx fork, and the whole reason the group
  // exists is that grandchildren can outlive the child. Whatever path leads here, the exit
  // condition is "the group is gone" — so every path ends at the same probe-and-escalate.
  ...
  // Signal 0 is a pure existence probe: true means something in the group survived the leader.
  // Survivors already had their grace — SIGKILL, and one short beat for the kernel to reap.
  if (signalGroup(managed.pid, 0)) {
    signalGroup(managed.pid, "SIGKILL");
    await delay(200);
  }
}
 ...
 */
// `0` is the null signal: no delivery, pure existence probe — the POSIX idiom for "is anything
