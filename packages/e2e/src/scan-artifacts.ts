...
export function describeScan(result: ScanResult): string {
  ...
  if (result.hits.length === 0) {
    if (result.unreadable.length > 0) {
      // Not "clean, with a footnote": a scan that could not read part of what it was pointed at
      // has not cleared it, and the summary must not read as if it had.
      return `${base} — ${result.unreadable.length} artifact(s) NOT SCANNED (treated as a failure)`;
    }
    return `${base} — clean`;
  }
