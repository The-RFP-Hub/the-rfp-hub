...
test.describe("M3-5 the signed-in dashboard", () => {
  ...
  test("a key's secret is shown once and is gone after a reload", async ({ page, stack }) => {
    ...
    // Registered AT MINT: this is the one key whose secret exists only in the page — no API-side
    // fixture ever sees it, so nothing else can register it, and an artifact scan that never
    // learned the value would call a screenshot carrying it clean.
    const minted = (await secretBlock.textContent())?.match(/rfph_[a-z0-9]+_[A-Za-z0-9_-]+/)?.[0];
    expect(
      minted,
