...
const fixture = (
  ...
  status: "open",
  sponsoringOrganizations: [{ name: "QC Org", slug: "qctest-org" }],
...
run("GET /v1/opportunities input contract", () => {
  ...
  beforeAll(async () => {
    ...
    for (const record of [
      fixture({ id: "qctest:grant", fundingType: "grant", grant: {}, tags: ["qc-a"] }),
      fixture({ id: "qctest:rfp", fundingType: "rfp", rfp: {}, tags: ["qc-b"] }),
    ]) {
  ...
  // blank. `ecosystem`/`tag`/`q` always accepted that; fundingType/status must not be the one pair
  // that 400s, or the same request passes or fails depending on which filter the user left empty.
  it("ignores an empty value on every list filter instead of 400ing", async () => {
    const blank = "fundingType=&status=&ecosystem=&network=&category=&tag=&organization=&q=";
