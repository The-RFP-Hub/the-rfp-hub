...
describe("readS3Config", () => {
  it("returns null without S3_BUCKET, so the local sink stays the default", () => {
    ...
    expect(readS3Config({ S3_BUCKET: "   ", AWS_REGION: "us-east-1" })).toBeNull();
  });
...
describe("createS3Sink", () => {
  ...
  // CDN applies its own origin default to the MOVING latest.* aliases and keeps serving the
  // previous dataset after a run, with no invalidation step anywhere.
