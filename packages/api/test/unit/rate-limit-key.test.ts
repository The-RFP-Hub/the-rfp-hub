/**
 * The address half of a rate-limit key. Behind a trusted proxy `request.ip` is attacker-written
 * text, so for every input: it never lands in `acct:`, and never becomes a bucket per connection.
 */
...
describe("addressBucket", () => {
  ...
  it("never lets a forwarded value reach the account namespace", () => {
    // The collision the prefix exists for; `acct:7` is also the shape of a host and a port.
...
describe("addressBucket behind a trusted proxy", () => {
  ...
  it("selects the hop the count names, and buckets it", async () => {
    // The address the trusted proxy saw, not the leftmost claim.
