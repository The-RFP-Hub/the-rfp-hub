...
| `VERIFY_NIGHTLY_LIMIT` | `500` | Entries one `verification-backfill` **invocation** checks — not one pass: a capped pass reports `remaining: 0` so `--passes` cannot multiply it. The TTL means the selection never drains, so this cap, not the predicate, is what bounds the nightly run |
