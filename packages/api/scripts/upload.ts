 ...
 * moving `latest.*` aliases would be served stale for that TTL after each run (there is no
 * invalidation step), while the archive objects would be revalidated against the origin for no
 * reason.
 *
 * `immutable` is only honest because the exporter names archive objects after a digest of their
 * own bytes (see scripts/export.ts): a key never designates two different datasets, so a year of
 * caching cannot serve content that has since been replaced. Do NOT hand it a key whose content
 * can change.
 */
export const CACHE_CONTROL = {
  /** A stable key, rewritten every run: revalidate often so a consumer sees today's dataset today. */
