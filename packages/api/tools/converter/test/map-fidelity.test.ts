...
import { type RegistryProgram, mapProgram, organizationNamesOf } from "../map-program.js";
import {
  UPSTREAM_PROGRAMS,
  grantProgram,
  hackathonProgram,
...
/**
 * The fallback is not a rare path — most upstream programs name no organisation at all — so the
 * two properties that keep it honest are asserted across every recorded upstream shape at once,
 * rather than on whichever single fixture happens to exercise them.
 *
 * This used to run against a committed snapshot of raw upstream rows. That snapshot is gone: the
 * repo's dataset is curated Standard documents now (`data/seed-corpus.json`), and a raw-shape file
 * kept alongside it purely to feed this suite would be a second, unserved copy of the data whose
 * only job was to make a test look broader than it is.
 */
describe("operating organizations across every recorded upstream shape", () => {
  ...
  it("publishes a real organisation whenever the upstream gives it one", () => {
    ...
    const byId = new Map(Object.values(UPSTREAM_PROGRAMS).map((p) => [p.programId ?? p.id, p]));
    const fabricated = mapped.filter((o) => o.operatingOrganizations[0]?.name === o.title);
    for (const o of fabricated) {
      const p = byId.get(o.id.slice(o.id.indexOf(":") + 1));
