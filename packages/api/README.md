...
| Staleness | `STALENESS_INACTIVE_DAYS`, `STALE_LISTING_REMINDER_DAYS`, `STALE_LISTING_REMINDER_CADENCE_DAYS` | Auto-close threshold plus stale-listing eligibility and rolling cooldown (defaults 90/60/30 days). Cooldown is per publisher account + organization and starts from the later of reminder creation and successful dispatch; in-flight/retryable rows overlap-block while terminally exhausted rows release the guard. Source verification or a publisher write refreshes `lastSeenAt`. Reminders require a current verified publisher membership. |
