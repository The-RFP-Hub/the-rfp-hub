...
| Email delivery | `EMAIL_TRANSPORT`, `EMAIL_FROM`, `AWS_SES_REGION`, `EMAIL_OUTBOX_DIR` | On the critical path of every login. `ses` in deployments (no credential — task-role IAM); the transports that do not deliver refuse to boot under `NODE_ENV=production`. |
