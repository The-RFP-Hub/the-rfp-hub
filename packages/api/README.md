...
|---|---|---|
| Authentication | `PRIVY_APP_ID`, `PRIVY_VERIFICATION_KEY`, `PRIVY_JWKS_URL`, `PRIVY_APP_SECRET` | The PEM verification key is the documented mechanism; `PRIVY_JWKS_URL` is an **unverified** override. `PRIVY_APP_SECRET` is used only by account enrichment, which is queued and never on the auth path. **No environment variable grants a role**: the first admin is made once by `pnpm --filter @the-rfp-hub/api grant-admin`, and every later one by an admin over the audited route. |
