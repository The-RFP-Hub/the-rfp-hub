 ...
 * (`?ecosystem=a&ecosystem=b`), comma-separate it (`?ecosystem=a,b`), or mix the two. Repeated in
 * every list parameter's description so the OpenAPI docs state it at the point of use.
 */
 ...
 * every other list filter (`ecosystem`, `category`, `q`, …) accepts that; rejecting it here alone would
...
export const listQuerySchema = {
  ...
  properties: {
    ...
    organization: {
      type: "string",
      description:
        "Organization slug. Matches ANY entry in operatingOrganizations OR sponsoringOrganizations (not only the primary operatingOrganizations[0] one).",
    },
