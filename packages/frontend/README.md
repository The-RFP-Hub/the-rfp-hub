...
| Variable | What it is |
|---|---|
| `NEXT_PUBLIC_API_URL` | **Required, every environment.** Origin of the API, e.g. `http://localhost:3001`. It is where `/v1` lives, where sign-in lives (`/api/auth`), and it is written into the page's CSP `connect-src`, so the browser may talk to this API and nothing else. |
...
`VERCEL_PROJECT_PRODUCTION_URL`, which the project's "Automatically expose System Environment
Variables" setting controls — or, anywhere else, from an explicit `NEXT_PUBLIC_SITE_ORIGIN`, which
also wins on Vercel when it is set (see "Environment" above). Neither deploy workflow trusts that
silently: `scripts/check-canonical-origin.mjs` runs against the pulled environment before the
build and fails production when it names no origin, and staging when it names one. Every other host this app
...
**Production is the apex: `https://rfpsear.ch`.** That hostname is also the Standard's canonical
...
`https://staging.rfpsear.ch` — a single label, by the certificate rule that gives the API
...
dependencies (`@the-rfp-hub/standard@^3.1.0`, `rfphub-validate@^0.3.1`). This
...
`--standard-spec` / `--validate-spec`). That HTTP request is only a fast pre-check, though:
