...
> **Maturity: `stable`** — declared 2026-09-25, and this directory is frozen (`FROZEN`). The
...
- **Spec version:** `1.0.1` — new entries carry `specVersion: "1.0.1"`. v1.0.1 is v1.0.0's
  contract under new identifiers, so an entry declaring `"1.0.0"` also conforms to this schema.
- **`$id`:** `https://rfpsear.ch/schemas/v1.0.1/opportunity.schema.json`
  — **canonical.** `rfpsear.ch` is the project's own domain and its apex is reserved for the
  spec, so this identifier is final for this version. It is stamped from
  [`spec.config.json`](../../spec.config.json), like every other identifier in the package.
  v1.0.0 keeps its identifiers on `ethrfps.app` ([`adr/0013`](../../../../adr/0013-move-spec-identity-to-rfpsear-ch.md)).
...
[`conformance/v1.0.1/pass/`](../../conformance/v1.0.1/pass) validates and every document in
[`conformance/v1.0.1/fail/`](../../conformance/v1.0.1/fail) does not. The suite asserts nothing
...
- **Canonical domain** — decided: `rfpsear.ch`, moved from `ethrfps.app` in
  [`adr/0013`](../../../../adr/0013-move-spec-identity-to-rfpsear-ch.md) and stamped from
  `spec.config.json`.
