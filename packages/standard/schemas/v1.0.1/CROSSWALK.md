...
Standard v1.0.1 maps onto both, so a consumer can translate between them.
...
| RFP Hub v1.0.1 | schema.org/Grant | DAOIP-5 (Grant Pool unless noted) |
|---|---|---|
| `id` | `identifier` | `id` |
| `title` | `name` | `name` |
...
| RFP Hub v1.0.1 | schema.org/Grant | DAOIP-5 |
|---|---|---|
...
- ✅ **JSON-LD `@context` shipped** — [`context.jsonld`](./context.jsonld), covering every
  ...
  Term IRIs are **versionless** (`https://rfpsear.ch/ns/rfp#`, canonical — the namespace carries
