...
| [`registries/*.json`](./registries) | The two open vocabularies whose values must mean the same thing across publishers: deadline labels and program models. Plus `entry.schema.json` constraining their shape. `ecosystems` is an open list with **no** registry — see the decline below. (`eligibility-keys` was retired 2026-08-05 with the field's move to free text — see `adr/0004`.) | Hand-authored entries; `registries/index.json` generated by codegen. |
...
| **A registry for `ecosystems`** | The field stays open with no registry (as did `networks`, until the field itself was removed on 2026-08-05). A registry over a list of chain names reads as an allowed-values list however carefully `NORMATIVE.md` words the "registered ≠ allowed" distinction, and it would put a review step in front of a newly launched chain for no interoperability gain. The two registries that remain govern vocabularies where two publishers genuinely need to mean the same thing by a value. |
