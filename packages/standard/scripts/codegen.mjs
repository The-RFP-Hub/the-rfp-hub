...
//   - schemas/<dir>/opportunity.schema.json  identity stamped in ($id, description)
...
// `specVersion` is an enum, and it is hand-written: codegen only insists the current one is in it.
{
  ...
  if (accepted.at(-1) !== spec.specVersion) {
    throw new Error(
      `codegen: specVersion enum ${JSON.stringify(accepted)} must end with '${spec.specVersion}'`,
    );
  }
}
...
const versionsIndex = {
  ...
  versions: readdirSync(p("schemas"), { withFileTypes: true })
    ...
    .map((dir) => ({
      ...
      path: dir,
      status:
        dir === spec.schemaDir.replace(/^schemas\//, "")
          ? spec.status
          : existsSync(p("schemas", dir, "FROZEN"))
            ? "stable"
            : "draft",
    })),
