...
| `skill` | Every file the documented install channels need is on GitHub `main` with the same sha256 as the audited local copy; the repository's own `scripts/check-skill.mjs` passes against that fetched copy; the fetched `scripts/search.mjs`, run against a corpus carrying an injected instruction in `summary`, `description`, `eligibility`, the operating organization's `description`, `fundingDetails.rfp.scopeOfWork` and a deadline `label`, emits only the keys `project()` names and no prose key at any depth (structural, so rewritten prose fails too) plus the instruction literal as a second line of defense; and a second run against a 400-character hostile `title` — the one third-party field the projection declares it KEEPS — gets it back as exactly the documented truncation (139 characters plus an ellipsis), with no `description` field. |
